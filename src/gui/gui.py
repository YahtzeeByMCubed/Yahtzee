...
from src.engine.yahtzee_env import IllegalActionError, GameOverError
...
# Ensure project root is in the path so `python -m src.gui.gui` standalone works.
PROJECT_ROOT = Path(__file__).resolve().parents[2]

if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from src.gui.misc import generate_random_yahtzee_game
