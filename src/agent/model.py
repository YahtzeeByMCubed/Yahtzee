...
class DQN(nn.Module):
    def __init__(self, state_dim: int = STATE_DIM, action_dim: int = ACTION_DIM) -> None:
        super().__init__()
        # TODO: build self.network = nn.Sequential(
    ...
    def forward(self, state: torch.Tensor) -> torch.Tensor:
        # TODO: return self.network(state). Output shape (batch, ACTION_DIM)
        # holding raw Q-values — masking happens downstream in DQNAgent.
        raise NotImplementedError
